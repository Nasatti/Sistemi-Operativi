...
public class UnfairSafeBridgeSem implements Bridge {
	...
	public void redEnter() {
		...
		// sezione critica
		if (nred + nblue == 0) {	// se il ponte è sgombro, l'auto passa
			nred++;
	...
	public void redExit() {
		...
		nred--;
		if (nr_waiting > 0) {	// ci sono auto rosse in attesa di entrare sul ponte
			rturn.release(); 	// sveglia un'auto rossa in attesa
			nred++;
			nr_waiting--;			
		}
		else if (nb_waiting > 0) {		// se ci sono auto blu in attesa
			bturn.release();
			nblue++;
			nb_waiting--;
		}
	...
	public void blueExit() {
		...
		nblue--;
		if (nb_waiting > 0) {	// ci sono auto blu in attesa di entrare sul ponte
			bturn.release(); 	// sveglia un'auto blu in attesa
			nblue++;
			nb_waiting--;			
		}
		else if (nr_waiting > 0) {		// se ci sono auto rosse in attesa
			rturn.release();
			nred++;
			nr_waiting--;
		}
