...
public class Database implements RWLock
{
   ...
   // flag to indicate whether the database is in use --> turno scrittore o lettore, se true scrive se false non scrive
   ...
   public synchronized void releaseReadLock(int readerNum)
   {
      ...
      if (readerCount == 0)
	 notify();// solo uno perché sveglia un solo scrittore
   ...
   public synchronized void releaseWriteLock(int writerNum)
   {
	 ...
	 * notify ALL of them.
	 */
      notifyAll();//All perché può svegliare tutti i lettori
   }
}
